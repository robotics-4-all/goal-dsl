...
class Condition(object):
    ...
    def build(self):
        ...
        self.cond_expr = self.condition.cond_expr
    ...
    def process_node_condition(cond_node):
        ...
        if textx_isinstance(
            ...
            # Build lambda
            cond_node.cond_expr = (OPERATORS[cond_node.operator])(
                cond_node.r1.cond_expr, cond_node.r2.cond_expr)
        else:
            operand1 = Condition.transform_operand(cond_node.operand1)
            operand2 = Condition.transform_operand(cond_node.operand2)
            cond_node.cond_expr = (OPERATORS[cond_node.operator])(operand1,
                                                                    operand2)

    def evaluate(self):
        # Check if condition has been build using build_expression
        if self.cond_expr not in (None, ''):
...
class InRangeCondition(AdvancedCondition):
    ...
    def process_node_condition(self, cond_node):
        ...
        cond_expr = (OPERATORS['InRange'])(operand1,
                                             cond_node.min,
                                             cond_node.max)
        cond_node.cond_expr = cond_expr
