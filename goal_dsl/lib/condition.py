    ...
    def evaluate(self):
        if self.cond_lambda not in (None, ""):
            try:
                ...
                goals = getattr(model, "goals_dict", {})
                if eval(
                    self.cond_lambda,
                    {"entities": entities, "goals": goals},
                    {
...
class InRangeCondition(AdvancedCondition):
    def __init__(self, parent=None, attribute=None, min=None, max=None, **kwargs):
...
class GoalStatusRef:
    def __init__(self, parent=None, goal=None, **kwargs):
        self.parent = parent
        self.goal = goal
